...
const Node = ({ icon, title, text }: NodeProps) => (
  <div className="flex items-center gap-3 bg-white/95 backdrop-blur-sm px-3 py-3 sm:px-4 rounded-xl shadow-md border border-sky-100 hover:shadow-lg transition-all duration-300 w-full max-w-[280px]">
    <div className="text-sky-600 text-lg sm:text-xl flex-shrink-0">{icon}</div>
    <div className="min-w-0">
      <h3 className="text-sm font-bold text-sky-900 leading-tight">{title}</h3>
...
const StarNode = ({
  ...
  const cardWrapperClass = mobile
    ? "relative w-full max-w-sm"
    : `absolute ${positionClasses[position]} z-20 w-[220px] sm:w-[240px] md:w-64`;

  const quoteMessage = encodeURIComponent(
    `Hi! I'm interested in ${title}. Please provide a quote for my business requirements.`
  );

  const getStartedMessage = encodeURIComponent(
    `Hi! I'm interested in ${title} (${price}). Please send me more details.`
  );

  const tailoredMessage = encodeURIComponent(
    "Hi! I'm interested in a custom solution. Please contact me to discuss my requirements."
  );
      ...
      <div
        className={`bg-white rounded-2xl shadow-xl border-2 transition-all duration-300 hover:shadow-2xl ${
          popular
            ...
            ? "border-emerald-300"
            : "border-sky-200"
        } ${mobile ? "p-4" : "p-4"} relative overflow-hidden`}
      >
        {popular && (
          <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[10px] sm:text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg whitespace-nowrap z-10">
            <FaStar size={10} /> MOST POPULAR
          </div>
        )}

        {isEcommerce && (
          <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-gradient-to-r from-emerald-500 to-green-500 text-white text-[10px] sm:text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg whitespace-nowrap z-10">
            <FaShoppingCart size={10} /> COMING SOON
          </div>
        )}

        <div className={`text-center ${popular || isEcommerce ? "mt-4" : "mt-1"}`}>
          <h3 className={`${mobile ? "text-base" : "text-lg"} font-bold text-sky-800`}>
            {title}
          </h3>

          <div className="mt-1">
            <span className={`${mobile ? "text-xl" : "text-2xl"} font-black text-sky-700`}>
              {price}
            </span>
          ...
          <p className="text-xs text-gray-600 mt-2 border-t border-gray-100 pt-2 leading-relaxed">
            {description}
          </p>
        </div>

        <ul className="text-xs text-gray-600 mt-3 space-y-1.5">
          {features.slice(0, mobile ? 3 : 3).map((feat, idx) => (
        ...
        {!isEcommerce && price !== "Tailored" && price !== "Quote Based" && (
          <a
            href={`[wa.me](https://wa.me/27672033731?text=${getStartedMessage})`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 inline-flex items-center justify-center gap-2 w-full bg-gradient-to-r from-sky-600 to-sky-700 hover:from-sky-700 hover:to-sky-800 text-white text-sm font-semibold py-2.5 rounded-xl transition-all duration-200"
          >
        ...
        {price === "Tailored" && (
          <a
            href={`[wa.me](https://wa.me/27672033731?text=${tailoredMessage})`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 inline-flex items-center justify-center gap-2 w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white text-sm font-semibold py-2.5 rounded-xl transition-all duration-200"
          >
        ...
        {price === "Quote Based" && (
          <a
            href={`[wa.me](https://wa.me/27672033731?text=${quoteMessage})`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 inline-flex items-center justify-center gap-2 w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white text-sm font-semibold py-2.5 rounded-xl transition-all duration-200"
          >
        ...
        {isEcommerce && (
          <button
            disabled
            className="mt-4 w-full bg-gray-200 text-gray-500 text-sm font-semibold py-2.5 rounded-xl cursor-not-allowed flex items-center justify-center gap-2"
          >
...
export default function Home() {
  const packages = {
    networkFix: {
      ...
      description:
        "Fix slow internet, unstable connections, and poor network performance.",
      features: [
    ...
    wifiCoverage: {
      ...
      description:
        "Eliminate dead zones and improve signal across your entire business.",
      features: [
    ...
    networkSetup: {
      ...
      description:
        "Professional network installation for offices and business environments.",
      features: [
    ...
    <>
      ...
      <section className="py-16 md:py-20 bg-white relative overflow-hidden">
        <div className="text-center mb-10 md:mb-12 px-4">
          <motion.h2
            ...
            className="text-3xl sm:text-4xl md:text-5xl font-bold text-sky-900 mb-3"
          >
          ...
          <motion.p
            ...
            className="text-gray-500 max-w-md mx-auto text-sm sm:text-base"
          >
        ...
        <div className="md:hidden px-4">
          <div className="flex flex-col items-center gap-4">
            <div className="w-24 h-24 bg-gradient-to-br from-sky-700 to-sky-800 text-white flex flex-col items-center justify-center rounded-full z-10 shadow-xl font-bold text-lg">
              HUB
            ...
            <Node
              ...
              text="Hands-on IT training"
            />
            <Node
              icon={<FaUsers />}
              title="Talent Platform"
              text="Connecting talent to opportunities"
            />
            <Node
              icon={<FaGlobe />}
        ...
        {/* Desktop ecosystem */}
        <div className="hidden md:flex relative items-center justify-center min-h-[460px] md:min-h-[520px]">
          <motion.div
            initial={{ scale: 0 }}
            whileInView={{ scale: 1 }}
      ...
      <section className="py-16 md:py-24 bg-gradient-to-b from-slate-50 via-white to-slate-50 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-20 md:top-32 left-5 md:left-10 w-72 md:w-96 h-72 md:h-96 bg-sky-100/20 rounded-full blur-3xl" />
          <div className="absolute bottom-20 md:bottom-32 right-5 md:right-10 w-72 md:w-96 h-72 md:h-96 bg-blue-100/20 rounded-full blur-3xl" />
        </div>
        ...
        <div className="lg:hidden px-4 relative z-10">
          <div className="flex flex-col items-center gap-5 max-w-md mx-auto">
            <div className="w-20 h-20 rounded-full bg-sky-700 text-white flex flex-col items-center justify-center shadow-xl">
              <span className="font-bold text-base">IT</span>
        ...
        {/* DESKTOP STAR NETWORK */}
        <div className="hidden lg:flex relative items-center justify-center min-h-[900px] lg:min-h-[1000px] max-w-7xl mx-auto">
          <motion.div
            className="w-20 h-20 sm:w-24 sm:h-24 lg:w-32 lg:h-32 bg-gradient-to-br from-sky-600 to-sky-700 text-white flex flex-col items-center justify-center rounded-full z-30 shadow-2xl border-4 border-white"
          ...
          >
            ...
            <a
              href="[wa.me](https://wa.me/27672033731?text=Hi!%20I%20need%20help%20with%20my%20business%20IT%20and%20network.%20Please%20assist%20me)"
              target="_blank"
              rel="noopener noreferrer"
          ...
          <div className="hidden lg:block absolute w-[680px] h-[680px] lg:w-[800px] lg:h-[800px] rounded-full border border-sky-200/50 pointer-events-none" />
        ...
        <div className="mt-12 md:mt-16 text-center px-4 relative z-10">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white px-5 sm:px-8 py-3 rounded-full font-semibold transition-all shadow-lg hover:shadow-xl text-sm sm:text-base"
          >
            <FaWhatsapp size={18} />
          ...
          <p className="text-xs text-gray-400 mt-3 max-w-md mx-auto">
            Get a Free Network Check - Find out how to improve your internet
            performance
          </p>
        </div>

        <div className="mt-8 md:mt-10 text-center px-4 relative z-10">
          <p className="text-xs text-gray-400 flex items-center justify-center gap-2 max-w-md mx-auto">
            <FaInfoCircle size={12} />
            Final pricing may vary depending on business size, equipment, and
            complexity
          </p>
        </div>
      </section>
